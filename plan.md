...
- `lib/gemini/image-generator.ts`: Gemini 3.1 Flash Image (`gemini-3.1-flash-image-preview`) ラッパー + `gemini-2.5-flash-image` フォールバック
...
- **画像生成の遅延**: Gemini 3.1 Flash Imageは3-6秒かかるため、水彩ディゾルブアニメーション（2-3秒）＋「おはなしを かきかえているよ...」等のメッセージで体感遅延を隠す
- **画像生成フォールバック**: プライマリ `gemini-3.1-flash-image-preview` → フォールバック `gemini-2.5-flash-image`。サーキットブレーカーパターンでエラー時に自動切替
